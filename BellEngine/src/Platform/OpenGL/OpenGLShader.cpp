...
namespace Bell
{
    ...
    std::string OpenGLShader::ReadFile(const std::string& filePath)
    {
        ...
        if (in)
        {
            ...
            {
                B_CORE_ERROR("Could not read file at {0}", filePath);
            }
        }
        else
        {
            B_CORE_ERROR("Could not open file '{0}'", filePath);
        }
    ...
    std::unordered_map<GLenum, std::string> Bell::OpenGLShader::PreProcess(const std::string& source)
    {
        ...
        // Find the type token that indicates the start of a shader
        ...
        while (pos != std::string::npos)
        {
            ...
            B_CORE_ASSERT(eol != std::string::npos, "Syntax error");
            // Step foward to the shader key word
            size_t begin = pos + typeTokenLength + 1;
            // Extract the shader key word
            std::string type = source.substr(begin, eol - begin);
            B_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type!");
    ...
    void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
    {
        ...
        if (isLinked == GL_FALSE)
        {
            ...
            B_CORE_ERROR("{0}", infoLog.data());
            B_CORE_ASSERT(false, "Shader link failure!");
            return;
        }
    ...
    void Bell::OpenGLShader::UploadUniformIntArray(const std::string& name, int* values, uint32_t count)
    {
        GLint location = glGetUniformLocation(m_RendererID, name.c_str());
